...
const inputBuscarNombreVentas = document.querySelector('#buscarProductoNombreVentas');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $("#buscarProductoNombreVentas").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: base_url + 'productos/buscarPorNombreVentas',
                dataType: "json",
                data: {
                    term: request.term
                },
        ...
        select: function (event, ui) {
            let precio = nombreKey == 'posCompra' ? ui.item.precio_compra : ui.item.precio_venta;
            agregarProducto(ui.item.id, 1, ui.item.stock, precio);
            inputBuscarNombreVentas.value = '';
            inputBuscarNombreVentas.focus();
            return false;
        }
    });
