...
class CreditosModel extends Query
{
    ...
    {
        $sql = "SELECT cr.*, cl.id AS id_cliente, cl.nombre, cl.telefono, cl.direccion FROM creditos cr INNER JOIN ventas v ON cr.id_venta = v.id INNER JOIN clientes cl ON v.id_cliente = cl.id WHERE cl.nombre LIKE '%" . $valor . "%' AND cr.estado = 1 LIMIT 10";
        return $this->selectAll($sql);
    }
    ...
    public function getCreditosActivosPorCliente($idCliente)
    {
        $sql = "SELECT cr.* FROM creditos cr 
            ...
            WHERE v.id_cliente = $idCliente AND cr.estado = 1 
            ORDER BY cr.id ASC"; // Orden cronológico
        return $this->selectAll($sql);
    }
    ...
    public function getDeudaTotalCliente($idCliente)
    {
        ...
        foreach ($creditos as $credito) {
            $abonado = $this->getAbono($credito['id'])['total'] ?? 0;
